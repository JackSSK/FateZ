...
### ery
p3 = pd.read_table('/storage/peiweikeLab/jiangjunyao/fatez/fine_tune/rebuild/data/GSE205117_ery/gene_cell.txt',header=0,index_col=0)
p4 = pd.read_table('/storage/peiweikeLab/jiangjunyao/fatez/fine_tune/rebuild/data/GSE205117_ery/gene_cell.txt',header=0,index_col=0)
### bp
p5 = pd.read_table('/storage/peiweikeLab/jiangjunyao/fatez/fine_tune/rebuild/data/GSE205117_bp/gene_cell.txt',header=0,index_col=0)
p6 = pd.read_table('/storage/peiweikeLab/jiangjunyao/fatez/fine_tune/rebuild/data/GSE205117_bp/gene_cell.txt',header=0,index_col=0)
...
class rebuild_baseline():
    ...
    def pp_pred(self,predict_X,predict_Y):
        predict_X = np.array(predict_X.T)
...
### training
ae = rebuild_baseline(X,Y)
ae.pp_train()
ae.train(epochs=500)
ae.test()

dnn = rebuild_baseline(X,Y,model_use='DNN')
dnn.pp_train()
dnn.train(epochs=500)
dnn.test()

### prediction atac value for other cell types
print('--------------dnn result!!!')
print('training cor all mean:',np.array(dnn.train_cor).mean())
print('predict endo atac')
dnn.pp_pred(p1,p2)
dnn.predict()
print('predict ery atac')
dnn.pp_pred(p3,p4)
dnn.predict()
print('predict bp atac')
dnn.pp_pred(p5,p6)
dnn.predict()
print('--------------ae result!!!')
...
ae.pp_pred(p5,p6)
ae.predict()
out1 = pd.DataFrame({'dnn':dnn.train_cor,'ae':ae.train_cor})
out1.to_csv('/storage/peiweikeLab/jiangjunyao/fatez/fine_tune/rebuild/result/baseline.csv')